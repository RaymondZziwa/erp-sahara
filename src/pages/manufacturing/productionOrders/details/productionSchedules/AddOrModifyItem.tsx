...
const AddOrModifyItem: React.FC<AddOrModifyItemProps> = ({
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const endpoint = item?.id
      ? MANUFACTURING_ENDPOINTS.PRODUCTION_SCHEDULES.UPDATE(id, item.id.toString())
      : MANUFACTURING_ENDPOINTS.PRODUCTION_SCHEDULES.ADD(id);
  ...
  return (
    <Dialog
      header={item?.id ? "Edit Production Schedule" : "Create Production Schedule"}
      visible={visible}
      className="w-[400px]"
      footer={footer}
      onHide={onClose}
