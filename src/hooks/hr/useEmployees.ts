...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/hr/employeesSlice.ts"; // Import actions from your data reducer
...
import { Employee } from "../../redux/slices/types/hr/Employee.ts";
...
const useEmployees = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<Employee[]>>(
        HUMAN_RESOURCE_ENDPOINTS.EMPLOYEES.GET_ALL,
        "GET",
        token.access_token
      );
