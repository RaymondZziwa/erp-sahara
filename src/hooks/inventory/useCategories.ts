...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/inventory/itemCategoriesSlice"; // Import actions from your data reducer
...
const useItemCategories = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<ItemCategory[]>>(
        INVENTORY_ENDPOINTS.ITEM_CATEGORIES.GET_ALL,
        "GET",
        token.access_token
      );
  ...
  const data = useAppSelector((state) => state.inventoryCategories);
