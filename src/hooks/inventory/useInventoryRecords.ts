...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/inventory/stockMovementSlice.ts"; // Import actions from your data reducer
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<Inventory[]>>(
        INVENTORY_ENDPOINTS.INVENTORIES.STOCK_MVT,
        "GET",
        token.access_token
      );
