...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/settings/modulePermissionSlice.tsx";
...
import { ModulePermission } from "../../redux/slices/types/settings/modulePermissions.ts";
...
const useModulePermissions = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<ModulePermission[]>>(
        SETTINGS_ENDPOINTS.PERMISSIONS.GET_ALL,
        "GET",
        token.access_token
      );
  ...
  const data = useAppSelector((state) => state.modulePermissions);
