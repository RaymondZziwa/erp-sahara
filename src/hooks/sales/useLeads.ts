...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/sales/leadsSlice.ts"; // Import actions from your data reducer
...
import { Lead } from "../../redux/slices/types/sales/Leads.ts";
...
const useLeads = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<Lead[]>>(
        SALES_ENDPOINTS.LEADS.GET_ALL,
        "GET",
        token.access_token
      );
