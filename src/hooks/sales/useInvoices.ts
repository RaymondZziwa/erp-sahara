...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/sales/invoiceSlice.ts"; // Import actions from your data reducer
...
import { Invoice } from "../../redux/slices/types/sales/invoice.ts";
...
const useInvoices = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<Invoice[]>>(
        SALES_ENDPOINTS.INVOICES.GET_ALL,
        "GET",
        token.access_token
      );
