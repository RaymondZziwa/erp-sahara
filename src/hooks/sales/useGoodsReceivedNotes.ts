...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/sales/goodsReceivedSlice.ts"; // Import actions from your data reducer
...
import { GoodsReceivedNote } from "../../redux/slices/types/sales/goodsReceived.ts";
...
const useGoodsReceivedNotes = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<GoodsReceivedNote[]>>(
        SALES_ENDPOINTS.GOODS_RECEIVED_NOTES.GET_ALL,
        "GET",
        token.access_token
      );
