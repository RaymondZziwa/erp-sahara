...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/accounts/payment_method"; // Import actions from your data reducer
...
const usePaymentMethods = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<PaymentMethod[]>>(
        API_ENDPOINTS.PAYMENT_METHODS.GET_ALL,
        "GET",
        token.access_token
      );
  ...
  const data = useAppSelector((state) => state.paymentMethods);
