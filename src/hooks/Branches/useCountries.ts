...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/Branches/CountrySlice.ts"; // Import actions from your data reducer
import useAuth from "../useAuth.ts";
import { apiRequest } from "../../utils/api.ts";

import { ServerResponse } from "../../redux/slices/types/ServerResponse.ts";
import { useAppDispatch, useAppSelector } from "../../redux/hooks.ts";
import { Country } from "../../redux/slices/types/Branches/type.ts";
...
const useCountries = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<Country[]>>(
        API_ENDPOINTS.COUNTRIES.GET_ALL,
        "GET",
        token.access_token
      );
