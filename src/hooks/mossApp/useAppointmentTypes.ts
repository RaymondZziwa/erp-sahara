...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/mossApp/appointmentTyesSlice.ts"; // Import actions from your data reducer
...
import { AppointmentType } from "../../redux/slices/types/mossApp/AppointmentType.ts";
...
const useAppointmentTypes = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await mossAppApiRequest<
        ServerResponse<AppointmentType[]>
      >(
        MOSS_APP_ENDPOINTS.APPOINTMENT_TYPES.GET_ALL,
        "GET",
        token.access_token
      );
  ...
  const data = useAppSelector((state) => state.appointmentTyes);
