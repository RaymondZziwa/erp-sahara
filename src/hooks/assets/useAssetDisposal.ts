...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/assets/assetDisposalSlice"; // Import actions from your assets reducer
import useAuth from "../useAuth";
import { apiRequest } from "../../utils/api";
import { ServerResponse } from "../../redux/slices/types/ServerResponse";
import { useAppDispatch, useAppSelector } from "../../redux/hooks";
import { ASSETSENDPOINTS } from "../../api/assetEndpoints";
import { AssetDisposal } from "../../redux/slices/types/mossApp/assets/asset";

const useAssetDisposal = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<AssetDisposal[]>>(
  ...
  const data = useAppSelector((state) => state.assetDisposal);

  // Return the data and a refresh function
  return { ...data, refresh: fetchDataFromApi };
};

export default useAssetDisposal