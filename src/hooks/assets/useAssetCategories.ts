...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/assets/assetCategoriesSlice"; // Import actions from your data reducer
...
const useAssetCategories = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<AssetCategory[]>>(
        ASSETSENDPOINTS.ASSETCATEGORIES.GET_ALL,
        "GET",
        token.access_token
      );
      dispatch(
  ...
  const data = useAppSelector((state) => state.assetCategories);
