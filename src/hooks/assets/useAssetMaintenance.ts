...
import {
  ...
  fetchDataSuccess,
  fetchDataFailure,
} from "../../redux/slices/assets/assetMaintenanceSlice"; // Import actions from your assets reducer
import useAuth from "../useAuth";
import { apiRequest } from "../../utils/api";
import { ServerResponse } from "../../redux/slices/types/ServerResponse";
import { useAppDispatch, useAppSelector } from "../../redux/hooks";
import { ASSETSENDPOINTS } from "../../api/assetEndpoints";
import { AssetMaintenance } from "../../redux/slices/types/mossApp/assets/asset";

const useAssetMaintenance = () => {
  ...
  const fetchDataFromApi = async () => {
    ...
    try {
      const response = await apiRequest<ServerResponse<AssetMaintenance[]>>(
        ASSETSENDPOINTS.ASSETS.MAINTENANCE.GET_ALL,
        "GET",
        token.access_token
      );
  ...
  const data = useAppSelector((state) => state.assetMaintenance);

  // Return the data and a refresh function
  return { ...data, refresh: fetchDataFromApi };
};

export default useAssetMaintenance